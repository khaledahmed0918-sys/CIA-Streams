  ...
  return (
    <div className="group/bell relative z-20">
      <button 
        ...
        className={`p-2 rounded-full transition-transform duration-200 ease-in-out active:scale-90 ${isSubscribed ? 'text-yellow-400' : 'text-gray-600 hover:text-black dark:text-gray-400 dark:hover:text-white'} disabled:opacity-50 disabled:cursor-not-allowed`}
        aria-label={tooltipText}
...
export const ScheduledStreamCard: React.FC<ScheduledStreamCardProps> = ({ schedule, onCardClick, isNotificationSubscribed, onNotificationToggle, notificationPermission, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <div 
      className="group relative min-h-[160px] rounded-2xl border border-black/10 dark:border-white/10 bg-white/40 dark:bg-white/5 p-5 text-black shadow-lg backdrop-blur-lg transition-all duration-300 ease-in-out hover:scale-[1.02] hover:shadow-2xl dark:text-white dark:bg-black/20 cursor-pointer"
      onClick={onCardClick}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="streamer-card-text" style={{ color: 'var(--text-streamer)' }}>
        ...
        <div className="relative z-10 flex flex-col h-full">
          {/* Status Badge */}
          <div className="absolute left-[-0.5rem] top-[-0.5rem] rtl:left-auto rtl:right-4 flex items-center gap-2 rounded-full border border-white/10 bg-black/80 px-3 py-1.5 text-xs font-semibold backdrop-blur-sm z-10" role="status">
          ...
          <div className="flex-grow my-4 text-center">
              <p className="font-semibold text-sm text-gray-800 dark:text-white dark:opacity-70">{t('liveIn')}</p>
              <p className="text-3xl font-bold tracking-wider text-black dark:text-white">{countdown.isOver ? "0s" : formatCountdown()}</p>
          </div>
              ...
              <div className="flex-1 min-w-0 text-sm">
                  {schedule.notes ? (
                      <Tooltip text={schedule.notes}>
                          <p className="truncate italic text-gray-700 dark:text-white dark:opacity-70">"{schedule.notes}"</p>
                      </Tooltip>
                  ) : (
